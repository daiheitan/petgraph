...
use {Directed, Direction, EdgeType, Incoming, Outgoing, Undirected};

use graph::node_index;
use graph::Graph;
use visit::{IntoEdgeReferences, IntoEdges, NodeCompactIndexable};
use visit::{IntoNodeIdentifiers, IntoNodeReferences, NodeCount, NodeIndexable};
...
#[cfg_attr(
    feature = "serde-1",
    serde(
        bound = "N: Hash + Eq + serde::Serialize + serde::de::DeserializeOwned, E: serde::Serialize + serde::de::DeserializeOwned"
    )
)]
